...
Nhắn tin 1-1 và nhóm real-time qua WebSocket, lịch sử tin nhắn, đếm tin nhắn chưa đọc. Composer `/chat` (`MessageArea.jsx`) tự giãn theo nội dung, có bảng emoji và đính kèm file: file đọc bằng FileReader thành data URL ngay ở client (≤ 3 MB/file, tối đa 5), nhúng vào `content` tin nhắn dưới dạng dòng `[[orbit-attachment]]{json}` — KHÔNG có endpoint upload / storage riêng; `SendMessageRequest.content` nới `max_length` lên 5_000_000 để chứa. `MessageBubble.jsx` parse marker này: ảnh render inline, file khác thành link `<a download>`. Layout `/chat` + `/assistant` có nhánh responsive cho điện thoại (chỉ CSS trong `@media`, desktop không đổi).
AI Agent chat: endpoint POST /api/v1/chat + /chat/resume, dùng LangGraph, 11 tool (search_messages — tìm tin nhắn cũ trong hội thoại theo từ khoá; list_tasks/list_memories — đọc Task/Memory thật của người dùng, phục vụ chính trang /assistant không gắn với hội thoại nào, tất cả đọc-only) — 4 tool có tác dụng phụ (create/update/delete_calendar_event, create_reminder) BẮT BUỘC đi qua interrupt() chờ người dùng xác nhận.
...
Chưa deploy online. Rate limiting đã có (`src/api/rate_limit.py` — in-memory sliding-window theo user/IP). Xem ROADMAP.md.
