...
> chứng minh giải pháp hoạt động**. Nó gộp lại phần đang nằm rải ở [README.md](README.md),
> [ROADMAP.md](ROADMAP.md), [SYSTEM_OVERVIEW.md](SYSTEM_OVERVIEW.md), [metric.md](metric.md) và
> [eval/](eval/) thành một mạch đọc từ vấn đề → nguyên tắc → giải pháp → số đo → kết quả → việc còn lại.
>
> **Phạm vi:** trợ lý AI cá nhân trong chat theo đúng [Frontend/detai.md](Frontend/detai.md). Hướng
> multi-agent theo phòng ban (Delivery / QA / Executive) là **future work** — xem [§8](#8-future-work),
> không phải nội dung nộp lần này.
>
...
- **Không** làm bản giả cho tính năng chưa xong. `Frontend/src/data/mockData.js` đã bị bỏ hẳn khỏi mọi
  import; cái gì chưa làm thì ghi rõ trong [ROADMAP.md](ROADMAP.md) và [§7](#7-hạn-chế-đã-biết).
...
`eval/`. [eval/EVALUATION_EVIDENCE.md](eval/EVALUATION_EVIDENCE.md) ghi rõ: *"This report never
...
- [metric.md §7](metric.md) định nghĩa cost benchmark: chọn cấu hình model **rẻ nhất vượt toàn bộ
...
|---|---|---|
| Vector DB (Qdrant / pgvector) lưu memory & lịch sử | `AsyncPostgresSaver` (agent thread) + tính năng **Memory** người dùng tự ghi + `search_messages` bằng `ILIKE` | Yêu cầu "memory hội thoại" (Cơ bản) đạt qua checkpointer; memory cá nhân đạt qua ghi chú người dùng. Chưa có nhu cầu semantic search đủ rõ để biện minh thêm một service + chi phí vận hành. Sẽ thêm nếu phát sinh nhu cầu thật ([§8](#8-future-work)). |
...
Khung đầy đủ: [metric.md](metric.md). Kết quả đo mới nhất: [eval/EVALUATION_EVIDENCE.md](eval/EVALUATION_EVIDENCE.md),
[eval/results/](eval/results/). Ánh xạ yêu cầu ↔ test ↔ code: [eval/TRACEABILITY_MATRIX.md](eval/TRACEABILITY_MATRIX.md).
...
| " | Overhead thêm vào lúc gửi tin nhắn (proactive async) | P95 < 300 ms | inference chạy async, ngoài critical path | `metric.md §2.6` |
...
| Proactive hữu ích | Suggestion precision | ≥ 0.90 | gate định nghĩa; acceptance rate lấy baseline ngày demo, mục tiêu pilot ≥ 30% | `metric.md §2.6` |

### 4.3 Cách chấm (tóm tắt — chi tiết ở metric.md)
...
Đối chiếu đầy đủ + việc còn lại theo ưu tiên: [ROADMAP.md](ROADMAP.md).
...
python scripts/eval_ragas.py               # grounding/faithfulness -> eval/results/ragas-latest.md
...
python scripts/generate_evaluation_evidence.py   # gộp -> eval/EVALUATION_EVIDENCE.md
```

Bằng chứng thủ công: [MANUAL_TEST_CASES.md](MANUAL_TEST_CASES.md) (kế hoạch),
[Deliverables/MANUAL_TEST_CASES.md](Deliverables/MANUAL_TEST_CASES.md) (10/10 PASS có ảnh, commit
`1990341`, 2026-08-16), video demo trong [Deliverables/](Deliverables/).
...
### 8.1 Multi-agent theo phòng ban *(hướng mở rộng — không thuộc bản nộp này)*

Tầm nhìn: Orbit thành ứng dụng AI nội bộ của **một công ty**, Admin tạo các **Workspace phòng ban**,
...
- Ba Workspace Agent dự kiến: **Product Delivery**, **Quality Assurance**, **Executive**.
- Personal Agent hiện tại là compatibility flow, **không** tính là Workspace Agent thứ tư.
- Foundation đã có baseline trong code (agent contracts, registry, deterministic router, scope
  resolver, resource guard, feature flag theo profile) + test, nhưng **specialist tools/prompt,
  pipeline publish `WorkspaceBrief`, runtime aggregation của Executive và UI chat từng Workspace
  chưa hoàn chỉnh** → chưa xem là tính năng hoàn tất.
- Thiết kế chi tiết: [docs/BRIEF.md](docs/BRIEF.md), [docs/PRD.md](docs/PRD.md),
  [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md),
  [docs/ENTERPRISE_WORKSPACE_FOUNDATION.md](docs/ENTERPRISE_WORKSPACE_FOUNDATION.md),
  [docs/MULTI_AGENT_IMPLEMENTATION_PLAN.md](docs/MULTI_AGENT_IMPLEMENTATION_PLAN.md).

### 8.2 Các hướng khác

1. **Deploy online thật** + CD (Render + Supabase + Vercel) — hạng mục lớn nhất còn thiếu; xem [DEPLOYMENT.md](DEPLOYMENT.md).
...
| Tài liệu | Nội dung |
|---|---|
| [Frontend/detai.md](Frontend/detai.md) | Đề bài gốc |
| [README.md](README.md) | Tính năng, cách chạy, "dùng thử" |
| [ROADMAP.md](ROADMAP.md) | Đối chiếu từng yêu cầu ↔ trạng thái + việc còn lại |
| [SYSTEM_OVERVIEW.md](SYSTEM_OVERVIEW.md) | Tổng quan hệ thống theo code |
| [metric.md](metric.md) | Khung metric & benchmark đầy đủ |
| [eval/EVALUATION_EVIDENCE.md](eval/EVALUATION_EVIDENCE.md) | Bảng bằng chứng release + số đo mới nhất |
| [eval/TRACEABILITY_MATRIX.md](eval/TRACEABILITY_MATRIX.md) | Yêu cầu ↔ test ↔ code ↔ evidence |
| [WORKLOG.md](WORKLOG.md) | Nhật ký từng thay đổi (lý do + cách verify) |
| [docs/](docs/) | Bộ thiết kế multi-agent (future work) |
