...
- **Nhắn tin 1-1 và theo nhóm, real-time**: tạo cuộc trò chuyện 1-1 hoặc nhóm (chọn nhiều người), gửi/nhận tin nhắn tức thời qua WebSocket, xem lại lịch sử tin nhắn, đếm tin nhắn chưa đọc, nút nhảy nhanh tới tin đầu tiên chưa đọc khi hội thoại có backlog dài. Mỗi hội thoại trong danh sách có công tắc bật/tắt quyền AI riêng (đồng bộ với badge trên header và AIPanel), không cần mở hội thoại ra mới đổi được. Ô soạn tin tự giãn theo nội dung, có bảng chọn emoji và đính kèm file (tối đa 5 file, mỗi file ≤ 3 MB): ảnh hiện inline trong bong bóng tin nhắn ở cả hai phía, file khác thành link tải — đính kèm được nhúng thẳng vào nội dung tin nhắn (data URL), chưa có kho lưu trữ file riêng. `/chat` và `/assistant` có layout riêng cho điện thoại (danh sách hội thoại kiểu Messenger, panel AI dạng tấm phủ toàn màn hình, composer không bị bàn phím ảo che).
...
- **AI Assistant cá nhân** (`/assistant`): khung chat riêng nối thẳng vào agent thật ở trên (không phải dữ liệu mẫu) — hỏi tự do, khi agent muốn tạo lịch/nhắc việc sẽ hiện nút Xác nhận/Huỷ ngay trong chat. Danh sách phiên chat bên trái và panel ngữ cảnh bên phải (task cần chú ý, lịch sắp tới, memory) đọc dữ liệu thật (`AssistantThread`) thay vì mock — bấm vào 1 phiên cũ sẽ tải lại đúng lịch sử hội thoại đó qua `thread_id`. Trả lời của AI hiển thị markdown thật (đậm/gạch đầu dòng/link) thay vì ký tự `**`/`-`/`#` thô.
...
- **Widget "AI credits" trong Sidebar**: đọc `GET /usage/status` thật (% ngân sách token đã dùng hôm nay) thay vì số hardcode cố định; endpoint này cố tình không trả cost/model — chỉ admin mới xem được chi tiết đó ở app admin.
- **Toast/confirm dùng chung** cho cả 2 app: lỗi API (đổi role, khoá tài khoản, xoá hội thoại/task/reminder/memory, phiên đăng nhập hết hạn...) hiện thông báo rõ ràng thay vì âm thầm không phản hồi hoặc dùng `window.confirm`/`alert` của trình duyệt.
...
14. Muốn thử **đính kèm file / emoji trong chat**: trong một hội thoại, bấm icon kẹp giấy ở ô soạn tin để chọn ảnh/file (≤ 3 MB, tối đa 5), hoặc icon mặt cười để chèn emoji, rồi gửi — ảnh hiện ngay trong bong bóng tin nhắn ở cả hai phía, file khác là link tải. Gõ đoạn dài nhiều dòng để thấy ô soạn tin tự cao dần (trần ~120px). Thu nhỏ cửa sổ xuống cỡ điện thoại (hoặc DevTools device mode) để kiểm tra `/chat` và `/assistant` trên màn hình hẹp.
...
## Sample queries — thử AI Assistant

Gõ trực tiếp vào `/assistant` (không cần chọn hội thoại nào) hoặc ô "Ask Orbit" trong AI Panel của 1 hội thoại cụ thể. Vài câu ví dụ theo từng nhóm tính năng thật:

**Đọc dữ liệu cá nhân** (`list_tasks`/`list_memories`, chỉ dùng được ở `/assistant`):
- "Hôm nay tôi có việc gì cần chú ý?"
- "Deadline nào sắp tới nhất?"
- "Bạn nhớ gì về cách tôi làm việc?"

**Tóm tắt / trích xuất từ 1 hội thoại cụ thể** (gõ trong AI Panel của hội thoại đó, cần đã Grant Permission):
- "Tóm tắt hộ tôi cuộc trò chuyện này."
- "Ai đang phải làm gì trong hội thoại này?"
- "Tuần này tôi có lịch gì?" (cần đã Connect Google Calendar)

**Tạo lịch/nhắc việc — luôn dừng lại chờ xác nhận trước khi tạo thật (human-in-the-loop):**
- "Đặt lịch họp team 3h chiều mai." → hiện thẻ Xác nhận/Huỷ, chỉ tạo sự kiện Google Calendar thật sau khi bấm Xác nhận.
- "Nhắc tôi gọi khách hàng lúc 5h chiều nay."
- "Đổi giờ họp team sang 4h chiều." (sửa sự kiện đã tạo trước đó)

**Tự động, không cần hỏi (proactive):** chỉ cần nhắn một tin nhắn thường có cam kết/lịch hẹn trong 1 hội thoại đã cấp quyền AI, ví dụ `"Mai 9h họp với team nhé"` — Orbit tự phát hiện và đẩy gợi ý vào `Tasks → AI suggestions`, không cần gõ lệnh gì.
